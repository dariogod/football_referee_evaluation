...
This script uses the refactored class-based role assignment implementations.
"""
...
from role_assigner_methods import (
...
# Mapping of method names to their classes
METHOD_CLASSES: Dict[str, Type[BaseRoleAssigner]] = {
    'dynamic_kmeans_silhouette': DynamicKMeansSilhouette,
...
def process_folder(folder: Path, methods: List[str], verbose: bool = False):
    ...
    for method_name in methods:
        ...
        # Get the role assigner class
        role_assigner_class = METHOD_CLASSES[method_name]
        role_assigner = role_assigner_class()
        ...
        for frame_id, frame_data in data.items():
            try:
                ...
                print(f"    Error processing frame {frame_id} with method {method_name}: {e}")
        
        # Save role assignments for this method
        output_file = folder / f"role_assignments_{method_name}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run role assignment on all color_assignments.json files using refactored methods'
    )
    parser.add_argument('--test_dir', default='test',
                       help='Path to the test directory (default: test)')
    parser.add_argument('--methods', nargs='+', 
                       choices=list(METHOD_CLASSES.keys()),
                       default=list(METHOD_CLASSES.keys()),
    ...
    for folder in test_folders:
        process_folder(folder, args.methods, args.verbose)
