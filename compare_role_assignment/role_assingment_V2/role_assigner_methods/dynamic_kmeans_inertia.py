...
class DynamicKMeansInertia(BaseRoleAssigner):
    """Dynamic K-means role assigner using inertia for goodness metric."""
    ...
    def perform_clustering(self, valid_persons: List[PersonWithJerseyColor], 
        ...
        """
        Perform K-means clustering with iterative outlier removal using inertia.
        ...
        for n_outliers in range(7):  # 0, 1, 2, 3, 4, 5, 6
            if len(current_indices) < 2:
                goodness_scores.append(float('inf'))  # For inertia, lower is better
            ...
            # Calculate goodness using inertia
            goodness = kmeans.inertia_
        ...
        # For inertia, we need to negate scores since lower is better
