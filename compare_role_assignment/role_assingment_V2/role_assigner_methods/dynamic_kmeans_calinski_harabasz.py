...
class DynamicKMeansCalinski(BaseRoleAssigner):
    """Dynamic K-means role assigner using Calinski-Harabasz score for goodness metric."""
    ...
    def perform_clustering(self, valid_persons: List[PersonWithJerseyColor], 
        ...
        """
        Perform K-means clustering with iterative outlier removal using Calinski-Harabasz score.
        ...
        for n_outliers in range(7):  # 0, 1, 2, 3, 4, 5, 6
            ...
            # Calculate goodness using Calinski-Harabasz score
            goodness = self._calculate_calinski_harabasz_score(current_X, labels)
        ...
        best_n_outliers = self._find_elbow_point(goodness_scores)
    ...
    def _calculate_calinski_harabasz_score(self, X: np.ndarray, labels: np.ndarray) -> float:
