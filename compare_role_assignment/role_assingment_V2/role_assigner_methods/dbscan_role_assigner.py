"""
DBScan Role Assigner - adapted from src/role_assigner.py

This role assigner implements the DBSCAN clustering algorithm with:
- Elbow method for automatic determination of optimal number of clusters
- Binary search to find the optimal eps parameter for DBSCAN
- Fallback to KMeans if DBSCAN doesn't converge
- Outlier detection for referees/goalkeepers
- Support for clustering in RGB, LAB, and HSV color spaces

The algorithm identifies the two largest clusters as teams and treats
smaller clusters or outliers as referees.
"""
...
class DBScanRoleAssigner(BaseRoleAssigner):
    ...
    def perform_clustering(self, valid_persons: List[PersonWithJerseyColor], 
        ...
        optimal_clusters = self._determine_optimal_clusters(X_colors)
        ...
        while max_eps - min_eps > 0.1:  # Convergence threshold
            ...
            labels = dbscan.fit_predict(X_colors)
        ...
        if best_labels is None:
            ...
            best_labels = kmeans.fit_predict(X_colors)
